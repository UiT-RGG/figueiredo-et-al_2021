...
imiquimod_identifying_outliers <- df %>%
  group_by(treatment, day, gene) %>%
  identify_outliers(ratio)  # identifying outliers in 'ratio' column

imiquimod_identifying_outliers <-
  full_join(df, imiquimod_identifying_outliers)  # joining outlier information with dataframe

# converting NAs from extreme column into FALSE
imiquimod_identifying_outliers["is.extreme"][is.na(imiquimod_identifying_outliers["is.extreme"])] <-
  FALSE

# subsetting all non-extreme outliers/values into a new dataframe
imiquimod_outliers_removed <-
  subset(
    imiquimod_identifying_outliers,
    imiquimod_identifying_outliers$is.extreme == FALSE
  )
###

## Organizing data by gene and day ####
imiquimod_correlation <- imiquimod_outliers_removed %>%
  group_by(gene, day) %>%
...
imiquimod_wide <- imiquimod_correlation %>%
...
imiquimod_formatted <- formatted_cors(imiquimod_wide)

# Plotting ####
P3 <- imiquimod_formatted %>%

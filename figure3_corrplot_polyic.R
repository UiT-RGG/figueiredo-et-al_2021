...
polyic_identifying_outliers <- df %>%
  group_by(treatment, day, gene) %>%
  identify_outliers(ratio)  # identifying outliers in 'ratio' column

polyic_identifying_outliers <-
  full_join(df, polyic_identifying_outliers)  # joining outlier information with dataframe

# converting NAs from extreme column into FALSE
polyic_identifying_outliers["is.extreme"][is.na(polyic_identifying_outliers["is.extreme"])] <-
  FALSE

# subsetting all non-extreme outliers/values into a new dataframe
polyic_outliers_removed <-
  subset(
    polyic_identifying_outliers,
    polyic_identifying_outliers$is.extreme == FALSE
  )
###

## Organizing data by gene and day ####
polyic_correlation <- polyic_outliers_removed %>%
  group_by(gene, day) %>%
...
polyic_wide <- polyic_correlation %>%
...
polyic_formatted <- formatted_cors(polyic_wide)

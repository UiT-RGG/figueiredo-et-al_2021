...
df <-
  ...
  df %>% filter(treatment == 'Hyaluronic acid') %>%
...
hyaluronica_identifying_outliers <- df %>%
  group_by(treatment, day, gene) %>%
  identify_outliers(ratio)  # identifying outliers in 'ratio' column

hyaluronica_identifying_outliers <-
  full_join(df, hyaluronica_identifying_outliers)  # joining outlier information with dataframe

# converting NAs from extreme column into FALSE
hyaluronica_identifying_outliers["is.extreme"][is.na(hyaluronica_identifying_outliers["is.extreme"])] <-
  FALSE

# subsetting all non-extreme outliers/values into a new dataframe
hyaluronica_outliers_removed <-
  subset(
    hyaluronica_identifying_outliers,
    hyaluronica_identifying_outliers$is.extreme == FALSE
  )
###

## Organizing data by gene and day ####
hyaluronica_correlation <- hyaluronica_outliers_removed %>%
  group_by(gene, day) %>%
...
hyaluronica_wide <- hyaluronica_correlation %>%
...
hyaluronica_formatted <- formatted_cors(hyaluronica_wide)

# Plotting ####
P2 <- hyaluronica_formatted %>%
